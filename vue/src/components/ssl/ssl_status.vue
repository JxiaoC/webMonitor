    ...
    <el-row align="middle" type="flex" class="name-id">
      ...
      <el-col :span="6">
        <el-tooltip placement="bottom" class="url" content="点击复制">
          <div class="host-copy" :data-clipboard-text="host" @click="copy">{{ host }}</div>
        </el-tooltip>
      </el-col>
      <el-col :span="2"
        ><div class="status">
...
export default {
  ...
  data: function () {
    return {
      ...
      currentRstDayRate:
        (this.rstDay / 90) * 100 > 100 ? 100 : (this.rstDay / 90) * 100,
      currentRstDayStr:
        this.rstDay >= 30
          ? "success"
          : this.rstDay >= 10
      ...
      currentRstDayTextColor:
        (this.rstDay / 90) * 100 >= 40 ? "color: #fff;" : "color: #000",
    };
  },
  watch: {},
  methods: {
    ...
    remove: function () {
      ...
      axios({
        ...
        url: window.host + "/ssl_monitor/remove",
        data: Qs.stringify({
          id: this.id,
        }),
      }).then(function () {});
    ...
    switchEnable: function (val) {
      ...
      axios({
        method: "post",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        url: window.host + "/ssl_monitor/edit",
        data: Qs.stringify({
          id: this.id,
          key: "enable",
          value: val,
    ...
    ref: function () {
      ...
      axios({
        ...
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        url: window.host + "/ssl_monitor/ref",
      }).then(function (res) {
        if (res.data.code != 0) {
          self.$notify.error({
            title: "错误",
...
#ssl .el-progress-bar__innerText {
  color: inherit;
}
</style>